...
def main():
    ...
    restaurant_distance, dict_id_index, dict_id_name = get_restaurant_distance(restaurant_data)
    # table = np.array(np.concatenate((np.array((dict_id_name.values())).reshape((len(dict_id_name), 1))
    # , my_ratings, restaurant_distance), axis=1))
    table = np.array(np.concatenate((my_ratings, restaurant_distance), axis=1))
    sys.stdout.write("Updated Ratings Table:\n\n %s \n\n" % table)
    ...
    sys.stdout.write("RESTAURANT SELECTED: %s" % dict_id_name[dict_id_index[np.argmax(restaurant_scores)]])
...
def get_restaurant_distance(restaurant_data):
    ...
    origin = np.array([[float(sys.argv[3]), float(sys.argv[4])]])
    ...
    return restaurant_distance, dict_id_index, dict_id_name
